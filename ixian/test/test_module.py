...
class TestModules:
    def test_load_module(self):
        ...
        from ixian.test.mocks.modules.functional import OPTIONS
        from ixian.test.mocks.modules.functional.tasks import TestTask
        from ixian.test.mocks.modules.functional.config import TestConfig
    ...
    def test_load_module_no_name(self):
        ...
        with pytest.raises(ModuleLoadError, match=".*: OPTIONS is missing 'name'"):
            load_module("ixian.test.mocks.modules.no_name")

    def test_load_module_invalid_path(self):
        """If module path is bad it can't be loaded"""
        with pytest.raises(ModuleNotFoundError):
            load_module("ixian.test.mocks.modules.does.not.exist")
        with pytest.raises(ModuleNotFoundError):
            load_module("ixian.test.mocks.modules.invalid-module-name")

    def test_load_module_no_options(self):
        """Module options are required"""
        with pytest.raises(ModuleLoadError, match=".*: missing OPTIONS"):
            load_module("ixian.test.mocks.modules.no_options")

    def test_load_module_no_config(self):
        """Config option is not required"""
        from ixian.test.mocks.modules.no_config import OPTIONS
        from ixian.test.mocks.modules.no_config.tasks import TestTask

        load_module("ixian.test.mocks.modules.no_config")
        assert resolve_task("test_task") is TestTask.__task__
    ...
    def test_load_module_config_path_bad_format(self):
        ...
        with pytest.raises(InvalidClassPath):
            load_module("ixian.test.mocks.modules.config_path_invalid")

    def test_load_module_config_path_does_not_exist(self):
        """Config path should exist"""
        with pytest.raises(ModuleNotFoundError):
            load_module("ixian.test.mocks.modules.config_path_does_not_exist")

    def test_load_module_no_tasks(self, mock_environment):
        """Module tasks are not required"""
        from ixian.test.mocks.modules.empty_tasks_module import OPTIONS
        from ixian.test.mocks.modules.empty_tasks_module.config import TestConfig

        load_module("ixian.test.mocks.modules.empty_tasks_module")
    ...
    def test_task_module_does_not_exist(self):
        ...
        with pytest.raises(ModuleNotFoundError):
            load_module("ixian.test.mocks.modules.task_path_does_not_exist")
    ...
    def test_task_load_error(self):
        ...
        with pytest.raises(Exception, match="Intentional Exception"):
            load_module("ixian.test.mocks.modules.task_load_error")
